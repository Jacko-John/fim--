...
function getCodeContext(){
    ...
    let suffixL = position.line + offset > editor.document.lineCount ? 
                    editor.document.lineCount : position.line + offset;
    ...
    for(let l = prefixL; l < suffixL; l++){
        ...
        if(l === position.line){
            text = text.slice(0, position.character) + "<$cursor$>" + text.slice(position.character);
        }
        rangeText += `${text}\n`;
