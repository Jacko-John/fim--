...
async function loadLanguage(langName: string) {
  const { Language } = require("web-tree-sitter");
	return await Language.load(path.join(__dirname, 'out', `tree-sitter-${langName}.wasm`))
}

function loadQuery(langName: LanguageName){
  switch(langName){
    case LanguageName.CPP: return cppQuery;
    case LanguageName.C_SHARP: return csharpQuery;
    case LanguageName.C: return cQuery;
    case LanguageName.PHP: return phpQuery;
    case LanguageName.TYPESCRIPT: return typescriptQuery;
    case LanguageName.TSX: return typescriptQuery;
    case LanguageName.JAVASCRIPT: return javascriptQuery;
    case LanguageName.PYTHON: return pythonQuery;
    case LanguageName.GO: return goQuery;
    case LanguageName.JAVA: return javaQuery;
    case LanguageName.RUBY: return rubyQuery;
    case LanguageName.RUST: return rustQuery;
    default:
...
export async function parseFile(editor: vscode.TextEditor) {
  ...
  try{
    ...
    if(!fileName.split(".").pop())
      return null;
    const languageName = supportedLanguages[fileName.split(".").pop() as string];
    let language = loadLanguage(languageName);
    language.then((lang: any) => {
      ...
      if(declarations.length !== 0){
        fileDeclarations[vscode.workspace.asRelativePath(editor.document.uri)] = new Array(...declarations);
      }
...
function findDeclarationName(root: any, fileContent: string): string[] {
  ...
  while (stack.length > 0) {
    ...
    while(parentStack.length > 0){
      ...
      if(node.startPosition.row >= currentParent.startPosition.row && node.endPosition.row <= currentParent.endPosition.row){
    ...
    if(GET_SYMBOLS_FOR_NODE_TYPES.includes(node.type)) {
      ...
      let offset = 0;
      while(!lines[node.startPosition.row + offset].includes(';') && !lines[node.startPosition.row + offset].includes('{') ){
        declarationText += `${lines[node.startPosition.row + offset]}\n`;
        offset++;
      }
      //存在 '}' 或 ';' 则在后续无需添加 '}' 进行闭合
      if(lines[node.startPosition.row + offset].includes('}') || lines[node.startPosition.row + offset].includes(';')){
...
function printNode(node: any){
  ...
  console.log(`Range: [${node.startPosition.row},${node.startPosition.column}] - [${node.endPosition.row},${node.endPosition.column}]`);
